...
fn make_client_info() -> ClientInfo {
    ...
    let configs: rfrp_config::config_info::base_types::ConfigInfo = serde_json::from_str(json).unwrap();
...
fn diag_compress_decompress_roundtrip() {
    ...
    println!("original len={}, compressed len={}, decompressed len={}",
             original.len(), compressed.len(), decomp_buf.len());
...
fn diag_crypto_roundtrip() {
    ...
    println!("plaintext len={}, ciphertext+nonce+tag len={}", plaintext.len(), buf.len());
...
fn roundtrip_register_frame() {
    ...
    match decoded {
        ...
        other => panic!("expected Register, got {:?}", other.map(|f| format!("{:?}", f))),
    }
}
